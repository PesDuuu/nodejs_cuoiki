...
const updateUserInformation = asyncMiddleware(async (req, res, next) => {
  ...
  await User.findByIdAndUpdate(
    userId,
    { phone, district, wards, streetAndHouseNumber, city },
    { new: true }
  );
...
const editUser = asyncMiddleware(async (req, res, next) => {
  const { name, city, phone, wards, district, streetAndHouseNumber } = req.body;
  const { id } = req.params;

  const user = await User.findByIdAndUpdate(
    id,
    { name, phone, wards, district, streetAndHouseNumber },
    { new: true }
  );
