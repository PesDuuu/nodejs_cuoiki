...
function changePass(password, newPassword, confirmPassword) {
  if (User) {
    fetch(`${http}/user/change-password`, {
      headers: {
        "Content-type": "application/json; charset=UTF-8",
        Authorization: `Bearer ${User?.token}`,
      },
...
window.addEventListener("load", function () {
  ...
  const products = {
    ...
    product_type: async function (page = 1, sort, type = "", limit = 16) {
      ...
      await fetch(
        `${http}/product?limit=${limit}&sortBy=${sortBy}&page=${page}`,
        {
          headers: {
            "Content-type": "application/json; charset=UTF-8",
            type,
    ...
    search: async function (search) {
      await fetch(`${http}/product/search/result`, {
        headers: {
      ...
      const productHtml = data.data.map((val, index) => {
        return `
            <div class="product col-lg-4 col-md-6 col-xl-3">
                ...
                <button data-id=${
                  val._id
                } class="add-cart">Thêm vào giỏ</button>
                </div>
                <div class="content">
