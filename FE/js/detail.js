...
function renderCmt(data) {
        ...
        <li class="rep">
          <div class="main-cmt">
            ...
            <div class="cmt-content">
              <p class="cmt-text"><a href="#">${
                v.repliedToUsername ? `@${v.repliedToUsername}` : ""
              }</a>${v.comment}</p>
            </div>
            <div class="cmt-command">
              ${deleteButton}
              <button class="btn-rep" data-id="${v._id}" data-parent="${
        v.parentCommentId
      }">Trả lời</button>
  ...
  // handling the event when pressing the 'xóa' button"
...
async function getComment() {
  ...
  await fetch(`${http}/comment/${productId}`, {})
    .then((data) => data.json())
    .then((data) => {
      renderCmt(data);
    })
  ...
  listCmt.addEventListener("click", function (e) {
    ...
    if (btnrep) {
      ...
      $$(".btn-rep").forEach((replyBtns) => {
        ...
        const currentModelNextSiblings =
          replyBtns.nextElementSibling.nextElementSibling;
